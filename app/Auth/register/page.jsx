...
const page = () => {
  ...
  const sendOtpEmail = async (email, otpCode, firstName) => {
    try {
      const response = await fetch('http://localhost:5000/api/send-otp-email', {
        method: 'POST',
        headers: {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      // Show the OTP code directly (since email service isn't set up yet)
      showNotification(`Your verification code is: ${otpCode}`, 'info')
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      // Show the new OTP code directly
      showNotification(`Your new verification code is: ${newOtpCode}`, 'info')
