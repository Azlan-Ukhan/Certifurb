...
const InboxPage = () => {
  ...
  const fetchEmails = async (folder = 'inbox', page = 1, search = '') => {
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/cms/emails?${queryParams}`);
  ...
  const fetchEmailStats = async () => {
    try {
      const response = await fetch('http://localhost:5000/api/cms/emails/stats');
      const data = await response.json();
      if (data.success) {
  ...
  const toggleStar = async (emailId, currentlyStarred) => {
    try {
      const response = await fetch(`http://localhost:5000/api/cms/emails/${emailId}`, {
        method: 'PUT',
        headers: {
  ...
  const handleBulkAction = async (action) => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/cms/emails/bulk', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
