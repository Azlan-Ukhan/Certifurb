...
const ShipmentsPage = () => {
  ...
  const fetchShipments = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/cms/shipments?${params}`);
  ...
  const handleUpdateShipment = async (e) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/cms/shipments/${editingShipment.id}`, {
        method: 'PUT',
        headers: {
