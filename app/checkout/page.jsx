...
const CheckoutPage = () => {
    ...
    try {
      ...
      if (cartData) {
        ...
        const validCartItems = cartWithDetails.filter(item => {
          ...
          console.log('Filtering item:', item, 'Has valid product:', hasProduct);
          return hasProduct;
        });
  ...
  useEffect(() => {
    // Load saved card info when user is available
    if (user) {
      loadSavedCardInfo();
    }
  }, [user]);
  ...
  const loadSavedCardInfo = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/get-card?userEmail=${user?.useremail}`);
      const data = await response.json();
  ...
  const handleSaveCard = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:5000/api/save-card', {
        method: 'POST',
        headers: {
  ...
  const saveOrderToDatabase = async () => {
    try {
      ...
      const orders = cartItems.map(item => ({
        ...
        totalPrice: getItemTotal(item.product, item.quantity),
        customerInfo: {
  ...
  const formatPrice = (price) => {
    return formatPriceUtil(price, false); // Use centralized formatter without PKR prefix
  };
          ...
          <div className="flex items-center gap-4">
            {isLoadingCart ? (
              ...
              <div className="flex items-center gap-3 bg-gray-50 p-3 rounded-lg">
                ...
                <div className="text-right">
                  <div className="font-bold text-gray-900">{formatPriceUtil(getSubtotal())}</div>
                </div>
              </div>
            ) : (
              <div className="flex items-center gap-3 bg-gray-50 p-3 rounded-lg">
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-6 sticky top-4">
                  ...
                  <div className="space-y-4">
                    {cartItems.map((item, index) => (
                      <div key={index} className="flex items-center gap-4">
                        ...
                        <div className="text-right">
                          <p className="font-bold text-gray-900">{formatPriceUtil(getItemTotal(item.product, item.quantity))}</p>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <div className="space-y-3 mb-6">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{formatPriceUtil(getSubtotal())}</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between text-lg font-bold">
                  <span>Total</span>
                  <span>{formatPriceUtil(getTotal())}</span>
                </div>
              </div>
              ...
              <div className="bg-green-50 border border-green-200 rounded-lg p-3 mb-6">
                <div className="flex items-center gap-2 text-green-700 text-sm">
                  <span className="font-medium">TOTAL SAVINGS {formatPriceUtil(500)}</span>
                </div>
              </div>
